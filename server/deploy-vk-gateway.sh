...
echo ""
echo "Дальше:"
echo "  1) В .env добавьте TELEGRAM_BOT_TOKEN и TELEGRAM_CHAT_ID (для /telegram-send)"
echo "  2) Nginx+SSL на api.ermakcentr.ru (см. server/nginx-vk-gateway.example.conf)"
echo "  3) App Platform: VITE_LEADS_SERVER_URL=https://api.ermakcentr.ru"
echo "     (с https://ermakcentr.ru без HTTPS шлюза браузер блокирует http://IP:5055)"
